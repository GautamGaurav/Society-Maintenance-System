...
export function mapBuilder(row) {
...
export function mapSite(row) {
    ...
    if (mapped.builder !== undefined && mapped.builder !== null) {
        ...
        const val = mapped.builder;
        const asNum = Number(val);
        if (!Number.isNaN(asNum)) {
            mapped.builderId = asNum;
            delete mapped.builder;
        }
        else if (mapped.builderId === undefined && typeof val === "string") {
            // keep as string if it was builderName
            mapped.builder = val;
        }
    }
    if (mapped.builderId === undefined && mapped.builderId === null) {
        // nothing
    }
    return mapped;
}
